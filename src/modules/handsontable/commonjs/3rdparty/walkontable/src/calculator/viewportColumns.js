...
var ViewportColumnsCalculator = function () {
  ...
  _createClass(ViewportColumnsCalculator, [{
    ...
    value: function refreshStretching(totalWidth) {
      ...
      } else if (Array.isArray(this.stretch)) {
        this.stretchEachRemainingSize = remainingSize;
        this.stretchEachColumnsWidth = [];
        this.needVerifyLastColumnWidth = true;
      }
    }
    ...
    value: function getStretchedColumnWidth(column, baseWidth) {
      ...
      if (this.stretch === 'all' && this.stretchAllRatio !== 0) {
        ...
        result = this._getStretchedEachColumnWidth(column, baseWidth);
      }

      return result;
    }
  }, {
    key: '_getStretchedEachColumnWidth',
    value: function _getStretchedEachColumnWidth(column, baseWidth) {
        if (typeof this.stretchEachColumnsWidth[column] !== "number") {
            var priv = privatePool.get(this);
            var totalColumns = priv.totalColumns;

            if (column === totalColumns - 1) {
                let remainingSize = this.stretchEachRemainingSize;
                for (const s of this.stretchEachColumnsWidth) {
                    remainingSize -= s;
                }
                this.stretchEachColumnsWidth[column] = remainingSize;
            } else {
                this.stretchEachColumnsWidth[column] = Math.round(this.stretchEachRemainingSize * this.stretch[column]);
            }
        }
        return baseWidth + this.stretchEachColumnsWidth[column];
    }
