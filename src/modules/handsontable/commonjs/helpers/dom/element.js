...
const trimmingContainerSpecialClass = `tc${Date.now().toString(36).substr(-4)}${Math.random().toString(36).substr(-4)}`;
const noTrimmingContainerSpecialClass = `ntc${Date.now().toString(36).substr(-4)}${Math.random().toString(36).substr(-4)}`;
...
exports.trimmingContainerSpecialClass = trimmingContainerSpecialClass;
exports.noTrimmingContainerSpecialClass = noTrimmingContainerSpecialClass;
...
function getScrollableElement(element) {
  ...
  while (el && el.style && document.body !== el) {
    ...
    if (el.className.indexOf(trimmingContainerSpecialClass) >= 0) {
        if (!el.translateScrollTo) {
            let parentScrollable = getScrollableElement(el);
            if (parentScrollable === window) {
                parentScrollable = window.document.documentElement;
            }
            el.translateScrollTo = parentScrollable;
        }
        return el;
    } else if (el.className.indexOf(noTrimmingContainerSpecialClass) >= 0){
        el = el.parentNode;
        continue;
    }
